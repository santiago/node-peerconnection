...
(function() {
  ...
  rtc.connect = function(server, room) {
    ...
    rtc._socket.on('open', function() {
      ...
      rtc._socket.on('message', function(msg) {
        console.log(msg)
        var json = JSON.parse(msg);
        rtc.fire(json.eventName, json.data);
      });

      rtc._socket.on('error', function(err) {
        console.log('onerror');
        console.log(err);
      });

      rtc._socket.on('close', function(data) {
...
rtc.connect('ws://localhost:6060');


// CLIENT
// var client = require('./lib/webrtc.io-client');
// client.connect('ws://localhost:6060');
...
function createPeerConnection() {
  ...
  var pc = new PeerConnection(pc_config);
